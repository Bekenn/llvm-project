...
QualType Sema::DeduceArgTypeForTST(TemplateParameterList *TemplateParams,
  ...
  ParamTST = ParamTST->getCanonicalTypeInternal()
      ->castAs<TemplateSpecializationType>();
  auto TemplateName = ParamTST->getTemplateName();
  ...
  if (isEquivalentToPrimary()) {
    ...
    SmallVector<TemplateArgumentLocInfo> ArgInfos(TSTArgs.size());
    TemplateSpecializationTypeLoc::initializeArgLocs(Context, TSTArgs,
        ArgInfos.data(), SourceLocation());
    SmallVector<TemplateArgumentLoc> Args(PrimaryTemplateParams->size());
    for (size_t I = 0, N = TSTArgs.size(); I != N; ++I)
      Args[I] = TemplateArgumentLoc(TSTArgs[I], ArgInfos[I]);
